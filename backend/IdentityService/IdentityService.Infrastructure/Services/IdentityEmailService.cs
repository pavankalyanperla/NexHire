...
public class IdentityEmailService
{
    ...
    public async Task SendWelcomeEmailAsync(string personalEmail, string companyEmail, string fullName, string tempPassword)
    {
        ...
        try
        {
            ...
            var body = $"""
                <div style="font-family:Arial,sans-serif;max-width:600px">
                  ...
                  <div style="background:#f0f7ff;padding:16px;border-radius:8px;margin:12px 0;border:1px solid #bfdbfe">
                    ...
                    <p style="margin:4px 0"><b>Login Email:</b> {companyEmail}</p>
            ...
            message.To.Add(new MailAddress(personalEmail, fullName));
            await client.SendMailAsync(message);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[IdentityEmailService] Welcome email failed to {personalEmail}: {ex.Message}");
        }
    }
}
